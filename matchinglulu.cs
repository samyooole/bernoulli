...
public class MatchingEngine
{
    ...
    public void DynamicMarketMakerRebate(decimal constant, decimal gradient, decimal rebatePercentage)
    {
        ...
        foreach (var hourGroup in _contractMap)
        {
            ...
            // Simulate settling additional volume provided by the market maker
            ...
            foreach (var ask in asks)
            {
                ...
                Console.WriteLine($"  Settled {settled} units additionally.");
            }
            ...
            foreach (var bid in bids)
            {
                ...
                Console.WriteLine($"  Settled {settled} units additionally on bids.");
            }
    ...
    public void SettleContractsAll()
    {
        foreach (var hourGroup in _contractMap)
        {
            string settlementHour = hourGroup.Key;
    ...
    public void PrintTotalRebateCost()
    {
        Console.WriteLine($"Total rebate cost incurred: {_totalRebateCost}");
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        decimal rebatePercentage = 0.005m; // 0.5% rebate

        engine.DynamicMarketMakerRebate(constant, gradient, rebatePercentage);

        // Print total rebate cost
        engine.PrintTotalRebateCost();

        // Print remaining unsettled contracts
        engine.PrintUnsettledContracts();
    }
}
